...
// export let waveOptions: IWaveOptions = {
//   viscosity: 340,
//   mouseDist: 200,
//   damping: 0.1,
//   amplitudeRange: 100,
...
export default class Curve {
  ...
  public render = () => {
    ...
    for (let i = 0; i <= arrayPoint.length - 1; i++) {
      if (gsap.ticker.frame % 200 === 0 && !this.mouseIsHoverButton) {
        TweenMax.to(arrayPoint[i], Curve.waveOptions.randomTransition, {
          random: Math.floor(Math.random() * Curve.waveOptions.randomRange),
        });

        TweenMax.to(arrayPoint[i], Curve.waveOptions.amplitudeTransition, {
          amplitude: Math.floor(Math.random() * Curve.waveOptions.amplitudeRange),
        });
      }
      const waveValue =
  ...
  public resize = () => {
    ...
    if (app) {
      this.origin = this.resizeOptions[app.state.currentScene](
